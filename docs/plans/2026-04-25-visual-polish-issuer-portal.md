...
- **2026-04-25 — Phase 0** worktree `feat/visual-polish-issuer-portal` created at `/home/naim/.openclaw/workspace/hydrax-app.visual-polish/` from `main` HEAD `23265ca`. Baseline gates green (3 packages build + issuer-portal typecheck/test/build all clean).
- **2026-04-25 — Phase 1** (commit `d5545e7`) `@hydrax/tenant-theme` expanded to 41 tokens: 12 colors (incl new `colorBgRaised`, `colorTextStrong`, `colorFocusRing`), 14 typography, 7 spacing, 3 radius, 2 shadow, 3 motion. `TOKEN_TO_CSS_VAR` keyed by `keyof TenantThemeTokens` (compile-time exhaustive). 7 tests pass (5 applyTheme + 2 ThemeProvider). Backward-compatible — Button/Card/AppShell unchanged.
- **2026-04-25 — Phase 2** (commit `c1d6c39`) 7 new `@hydrax/ui` primitives: `Stack` (1D flex with token-driven gap/align/justify/wrap), `Heading` (display/h1/h2 with `as` element override), `Text` (body/bodySm/mono with default/muted/strong/danger/success tones), `Skeleton` (role=status loading shimmer), `EmptyState` (icon-or-image + heading + optional body + optional action), `NavItem` (button or anchor with active state via `aria-current`), `Avatar` (deterministic initials, no remote fetch). 31 tests pass (26 new + 5 existing). One principled fix: `Avatar.initials("")` early-return for empty string (the spec's `parts.length === 0` guard was unreachable because `"".split(/\s+/)` yields `[""]`).
- **2026-04-25 — Phase 3** (commit `2a11519`) `<AppShell>` upgraded with `brand` slot (56px sidebar header), `sidebarFooter` slot (bottom-pinned), and globally-mounted `@keyframes hydrax-skeleton-shimmer`. Backward-compatible — old `topbar` + `sidebar` props still work. 6 AppShell tests + 31 primitive tests = 37 ui tests pass; all 5 apps still typecheck/test/build green.
- **2026-04-25 — Phase 4** (commit `1b1e3ee`) nano-banana hero asset generated via Gemini 3.1 flash image preview through OpenRouter. Output: 1200×896 JPEG, 344 KB, minimalist clipboard + data grid + pen line art, cyan accent on near-black, no text/gradients/glow. Saved to `web/packages/ui/src/assets/issuer-empty-state.jpg` with provenance in `assets-meta.json`. 2 metadata smoke tests added; 37 ui tests pass. JPEG instead of plan's PNG — Gemini's choice; accepted because no transparency required.
- **2026-04-25 — Phase 5** (commit `da99c6d`) `@hydrax/issuer-portal` Home polished: `IssuerSidebar` (6 NavItems: Home, Products, Approvals, Investors, Activity, Settings — only `/` has a route handler, rest are intentional non-route stubs), `IssuerBrand` (lucide `Building2` + label), `IssuerTopBar` (search placeholder, notifications button, initials Avatar). `HomeRoute` renders Heading + intro + 3 stat tiles (literal `--`, no fake data) + Recent activity Card toggling between `<Skeleton>` (when `connected={true}`) and `<EmptyState>` with the hero JPEG (default). 8 issuer-portal tests pass; dev server smoke HTTP 200 on :5173. Two adjustments from spec: (a) `web/apps/issuer-portal/src/img.d.ts` ambient module decl needed for `*.jpg`/`*.png`; (b) import path resolved relatively (`../../../../packages/ui/src/assets/issuer-empty-state.jpg`) because `@hydrax/ui`'s `package.json` `exports` only declares `./dist/index.js`.
- **2026-04-25 — Phase 6** (no commit, verification only) all 4 other apps still typecheck/test/build green: distributor-portal, investor-portal, ops-console, admin each have 2 App tests passing and emit dist/. Bundle sizes ~235 KB (slight uptick from ~232 KB pre-Phase-3, attributable to upgraded AppShell consuming new CSS-var styling).
- **2026-04-25 — Phase 7** CLAUDE.md "Web Monorepo — Invariants" section extended with 5 new bullets covering token surface, primitive list, AppShell slots, and imagery sourcing rule. Plan doc verification log appended (this section).
